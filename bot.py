...
def normalize_chat_id(value: str) -> str | int:
...
def menu_kb():
    ...
    if cfg_bool("enabled_menu_goods", True):
        rows.append([InlineKeyboardButton(text=get_cfg("btn_menu_goods", "📦 Наши готовые товары"), callback_data="menu:goods")])
    if not rows:
    ...
    if ref:
        try:
            if ref.startswith("http://") or ref.startswith("https://"):
                sent = await message.answer_photo(photo=ref, caption=text, reply_markup=keyboard)
            else:
                ref_path = Path(ref)
                if ref_path.exists() and ref_path.is_file():
                    sent = await message.answer_photo(photo=FSInputFile(str(ref_path)), caption=text, reply_markup=keyboard)
                else:
                    sent = await message.answer_photo(photo=ref, caption=text, reply_markup=keyboard)
        except Exception:
...
async def render_step(cb: CallbackQuery, state: FSMContext, step: str, from_back: bool = False):
    ...
    if step == "print_tech":
        ...
        await send_step_cb(cb, get_cfg("text_print_tech", "🖨 Выберите технологию печати:\n\nКаждый метод подходит для разных задач.\nМы подскажем, что лучше для вашего проекта!"), kb(rows), photo_ref_for("photo_print"))
    elif step == "scan_type":
        rows = []
        ...
        rows.append(nav_row(False))
        await send_step_cb(cb, get_cfg("text_scan_type", "📡 3D-сканирование — перенос реальных объектов в цифру.\n\nВыберите тип объекта:"), kb(rows), photo_ref_for("photo_scan"))
    elif step == "idea_type":
        rows = []
        ...
        rows.append(nav_row(False))
        await send_step_cb(cb, get_cfg("text_idea_type", "✏️ У вас есть идея, но нет 3D-модели?\n\nВыберите направление:"), kb(rows), photo_ref_for("photo_idea"))
    elif step == "goods_type":
        rows = []
        if cfg_bool("enabled_goods_sport", True):
            rows.append([InlineKeyboardButton(text=get_cfg("btn_goods_sport", "🏆 Спорт (Chel_Arm)"), callback_data="set:goods_type:Спорт")])
        if cfg_bool("enabled_goods_decor", True):
            rows.append([InlineKeyboardButton(text=get_cfg("btn_goods_decor", "💡 Интерьер и декор"), callback_data="set:goods_type:Интерьер и декор")])
        if cfg_bool("enabled_goods_accessories", True):
            rows.append([InlineKeyboardButton(text=get_cfg("btn_goods_accessories", "🔑 Аксессуары"), callback_data="set:goods_type:Аксессуары")])
        if cfg_bool("enabled_goods_gaming", True):
            rows.append([InlineKeyboardButton(text=get_cfg("btn_goods_gaming", "🎮 Для геймеров"), callback_data="set:goods_type:Для геймеров")])
        if cfg_bool("enabled_goods_other", True):
            rows.append([InlineKeyboardButton(text=get_cfg("btn_goods_other", "🤔 Другое"), callback_data="set:goods_type:Другое")])
        rows.append(nav_row(False))
        await send_step_cb(cb, get_cfg("text_goods_type", "🛍 Наши готовые товары — разработки собственного производства.\n\nВыберите категорию:"), kb(rows), photo_ref_for("photo_goods"))
    elif step == "about":
        rows = []
        ...
        rows.append(nav_row(False))
        await send_step_cb(cb, get_cfg("about_text", "🏢 Chel3D — команда специалистов в области передовых технологий 3D-печати и разработки конструкций.\n\nПолный цикл работ: от эскиза до готового изделия!\n\nВыберите раздел:"), kb(rows), photo_ref_for("photo_about"))
...
async def on_menu(cb: CallbackQuery, state: FSMContext):
    try:
        ...
        step = {"print": "print_tech", "scan": "scan_type", "idea": "idea_type", "goods": "goods_type"}[branch]
...
async def on_file(message: Message, state: FSMContext, bot: Bot):
    ...
    if message.document:
        ...
        mime = doc.mime_type
        is_image_doc = (mime or "").startswith("image/")
        if ext not in {"stl", "3mf", "obj"} and not is_image_doc:
            await send_step(message, "Для документов поддерживаются STL/3MF/OBJ. Фото можно отправлять как фото или как изображение-документ.")
            return
        file_id = doc.file_id
        file_name = doc.file_name or ("image.jpg" if is_image_doc else "model")
        size = doc.file_size
    elif message.photo:
...
async def on_submit(cb: CallbackQuery, state: FSMContext, bot: Bot):
    try:
        ...
        if orders_chat_id:
            try:
                await send_order_to_chat(
                    bot,
                    normalize_chat_id(orders_chat_id),
                    order_id,
                    summary,
                    cb.from_user.id,
