...
async def render_step(cb: CallbackQuery, state: FSMContext, step: str, from_back: bool = False):
    ...
    if step == "print_tech":
        await send_step_cb(cb, get_cfg("text_print_tech", "Выберите технологию печати:"), kb([
        ...
        await send_step_cb(cb, get_cfg("text_scan_type", "Что нужно отсканировать?"), kb([
        ...
        await send_step_cb(cb, get_cfg("text_idea_type", "Выберите категорию:"), kb([
...
async def on_set(cb: CallbackQuery, state: FSMContext):
    ...
    if key == "technology":
        ...
        if value.startswith("🤔"):
            ...
            await send_step_cb(cb, get_cfg("text_attach_file", "Прикрепите STL/3MF/OBJ как документ или нажмите ❌ У меня нет файла"), kb([
...
async def on_text(message: Message, state: FSMContext):
    ...
    if waiting == "other_material":
        ...
        await persist(state)
        await send_step(message, get_cfg("text_attach_file", "Прикрепите STL/3MF/OBJ как документ или нажмите кнопку ниже."), kb([
            [InlineKeyboardButton(text="❌ У меня нет файла", callback_data="set:file:нет")],
            nav_row(),
        ]))
...
async def on_submit(cb: CallbackQuery, state: FSMContext, bot: Bot):
    try:
        ...
        await send_step_cb(cb, get_cfg("text_submit_ok", "Спасибо! Заявка отправлена менеджеру ✅"), menu_kb())
        await state.clear()
    except Exception:
